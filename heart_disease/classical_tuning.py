...
# Sensitivity = recall of disease class    (1)
# Specificity = recall of no-disease class (0)
...
# Model selection is based on cross-validated ROC-AUC,
# not test-set accuracy, to keep the test set untouched
...
cv_results_df.to_csv("classical_cv_results.csv", index=False)
...
final_results.to_csv("final_classical_model_results.csv", index=False)


# The pipeline contains BOTH StandardScaler + final model,
# so we can save them together as a single artifact.

joblib.dump(best_model, "heart_disease_classical_model.joblib")


feature_info = pd.DataFrame({"Feature": X_train.columns})
feature_info.to_csv("model_features.csv", index=False)
...
print("3. heart_disease_classical_model.joblib")
